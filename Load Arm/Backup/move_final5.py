...
class YoloOrderRobotNode(Node):
    ...
    def main_loop(self):
        ...
        for (x1, y1, x2, y2), conf, label_name in picked_boxes:
            ...
            self.mc.send_angles(self.home_angles, 50)
            time.sleep(0.5)
            self.mc.send_coords([-105.0, 109.2, 318.6, -76.34, -49.66, -137.43], 70, 1)
            time.sleep(0.5)
            self.mc.send_coords([-238.5, 95.2, 208.4, -164.79, -26.14, -163.07], 70, 1)
            time.sleep(0.3)
            self.mc.send_coords([-237.2, -104.4, 225.4, -167.07, -26.75, -130.32], 50, 1)
            time.sleep(1)
            self.mc.set_gripper_value(70, 50)
            self.mc.send_coords([-197.2, -142.0, 274.6, -168.54, -38.25, -132.49], 50, 1)
            time.sleep(0.5)
            # 수량 카운트
