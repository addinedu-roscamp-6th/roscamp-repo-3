...
def estimate_angle_from_roi(bgr, box_xyxy, use_otsu=True, fixed_thr=200, debug_draw=False):
    """
    ROI에서 minAreaRect 기울기 추정.
    - Otsu 정/역 둘 다 시도해서 '길쭉함(aspect)'*'면적'이 큰 컨투어를 선택
    - 실패/애매하면 Canny 에지 PCA로 백업
    반환: (angle_deg or None, debug_image or None, area, aspect)
    """
    H, W = bgr.shape[:2]
    x1, y1, x2, y2 = [int(v) for v in box_xyxy]

    # ROI margin을 넉넉히 (기존 0.15 → 0.22)
    rx1, ry1, rx2, ry2 = _roi_from_box(x1, y1, x2, y2, W, H, margin_ratio=0.22)
    ...
    def _best_from_thresh(thr_img):
        ...
        for c in cnts:
            ...
            score = area * (aspect - 1.0)      # 길쭉+큰 것 선호
            if score > best_score:
                best_score = score
    ...
    if cand:
        ...
        if debug_draw:
            ...
            cv2.putText(dbg_img, f"angle={angle:.1f} deg", (8, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (50, 220, 50), 2)
            cv2.putText(dbg_img, f"area={area:.0f} ar={aspect:.2f}", (8, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (50, 200, 200), 1)
    ...
    if xs.size >= 50:
        ...
        v = eigenvectors[0]  # 주성분
        angle = math.degrees(math.atan2(v[1], v[0]))  # 이미지 좌표 기준
        # -90~90 정규화
        if angle > 90: angle -= 180
        if angle < -90: angle += 180
        if debug_draw:
            dbg_img = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
            cv2.putText(dbg_img, f"PCA={angle:.1f} deg", (8, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 50), 2)
        # 면적/종횡비는 모름 → None으로
        return float(angle), dbg_img, None, None

    return None, (roi if debug_draw else None), None, None
                ...
                while rclpy.ok() and self._ok and (not self.is_order_done()):
                    ...
                    if angle_deg is not None:
                        ...
                        if (a_area is None or a_aspect is None) or ((a_area >= self.angle_min_area) and (a_aspect >= self.angle_min_aspect)):
                            ...
                            a_cmd = float(np.clip(angle_filt + self.angle_offset, self.angle_min, self.angle_max))
