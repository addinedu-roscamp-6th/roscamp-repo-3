...
# # def estimate_angle_from_roi(bgr, box_xyxy,
# #                             use_otsu=False,          # 기본: 고정 임계
# #                             fixed_thr=250,           # 기본 임계=250
# #                             debug_draw=False,
# #                             margin_ratio=0.22,
# #                             offset=0.0):
# #     """
# #     ROI에서 물체 기울기 추정.
# #     - 보정(오프셋, w<h 처리)은 반드시 `_minarea_angle(rect, offset)`에서만 수행.
# #     - 여기서는 추가 보정/정규화 없음.
# #     - debug_draw=True면 imshow 3창: angle_gray / angle_thr / angle_debug
# #     반환: (angle_deg or None, debug_image or None, area, aspect)
# #     """
# #     H, W = bgr.shape[:2]
# #     x1, y1, x2, y2 = [int(v) for v in box_xyxy]
...
# #     if roi.size == 0:
# #         if debug_draw:
# #             cv2.imshow("angle_gray", np.zeros((1,1), np.uint8))
# #             cv2.imshow("angle_thr",  np.zeros((1,1), np.uint8))
# #             cv2.imshow("angle_debug", np.zeros((1,1,3), np.uint8))
# #             cv2.waitKey(1)
# #         return None, None, None, None
...
# #             score = area * (aspect - 1.0)  # 크고 길쭉한 것 선호
# #             if score > best_score:
# #                 best_score = score
# #                 best = (c, rect, area, aspect)
# #         return best  # or None
...
# #         cand2 = _best_from_thresh(thr_inv)
# #         if cand1 and cand2:
# #             score1 = cand1[2] * (cand1[3] - 1.0)
# #             score2 = cand2[2] * (cand2[3] - 1.0)
# #             cand = cand1 if score1 >= score2 else cand2
# #             dbg_src = thr if cand is cand1 else thr_inv
# #         else:
# #             cand = cand1 or cand2
# #             dbg_src = thr if cand is cand1 else thr_inv
...
# #     if cand:
# #         c, rect, area, aspect = cand
# #         angle = _minarea_angle(rect, offset=offset)  # ★ 보정은 여기서만
...
# #     # 5) (백업) PCA 경로 — 요청대로 보정은 여기 적용하지 않음
# #     edges = cv2.Canny(gray, 50, 150)
# #     ys, xs = np.where(edges > 0)
# #     if xs.size >= 50:
# #         pts = np.column_stack((xs.astype(np.float32), ys.astype(np.float32)))
...
# #         dbg_img = None
# #         if debug_draw:
# #             cv2.imshow("angle_thr", edges)  # thr 대신 edge 표시
# #             dbg_img = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
# #             cv2.putText(dbg_img, f"PCA={angle:.1f} deg", (8, 20),
# #                         cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 50), 2)
# #             cv2.imshow("angle_debug", dbg_img)
...
# #     # 실패 시 표시(선택)
# #     if debug_draw:
# #         cv2.imshow("angle_thr", np.zeros_like(gray))
# #         cv2.imshow("angle_debug", np.zeros((gray.shape[0], gray.shape[1], 3), dtype=np.uint8))
# #         cv2.waitKey(1)
# #     return None, None, None, None
...
#         # gp('angle_offset', 0)            # 각도 보정(현장 보정용, deg)
...
#         gp('angle_alpha', 0.4)            # EMA 계수(0.2~0.6 권장)
#         gp('angle_jump_limit', 30.0)      # 한 번에 허용할 최대 변화량(deg)
...
#                     chosen = None
#                     for _, rc in df.sort_values(['ymin','confidence'], ascending=False).iterrows():
...
#                         else:
#                             # 첫 픽이면 confidence 기준
#                             rc = df.sort_values(['confidence'], ascending=False).iloc[0]
#                             cx_c = float((rc.xmin + rc.xmax) * 0.5)
#                             cy_c = float((rc.ymin + rc.ymax) * 0.5)
#                             und_cand = undistort_pts(np.array([[cx_c, cy_c]], np.float32), K, dist)
#                             base_xy_cand = cv2.perspectiveTransform(und_cand.reshape(1,1,2), H_px2base)[0,0]
...
#                         # 품질 체크
#                         if (a_area is None or a_aspect is None) or ((a_area >= self.angle_min_area) and (a_aspect >= self.angle_min_aspect)):
#                             if self._angle_prev is not None:
#                                 angle_deg = _unwrap_to_prev(angle_deg, self._angle_prev)
#                                 delta = angle_deg - self._angle_prev
#                                 if abs(delta) > self.angle_jump_limit:
#                                     angle_deg = self._angle_prev + np.sign(delta) * self.angle_jump_limit
#                             angle_filt = _ema(self._angle_prev, angle_deg, self.angle_alpha)
#                             self._angle_prev = angle_filt
#                             a_cmd = float(np.clip(angle_filt, self.angle_min, self.angle_max))
#                             log.info(f'angle_raw={angle_deg:.1f}°, filt={angle_filt:.1f}°, cmd={a_cmd:.1f}° '
#                                      + (f'(area={a_area:.0f}, ar={a_aspect:.2f})' if (a_area and a_aspect) else '(PCA)'))
#                         else:
#                             log.info(f'angle 품질 낮음 → skip (area={a_area:.0f}, ar={a_aspect:.2f})')
#                     else:
...
#                             wait_until_stop(); time.sleep(0.6)
#                             _rot = mc.get_coords()
...
#                     mc.send_coords(goal_a, self.speed_approach, 0); wait_until_stop()
...
#                     mc.send_coords(approach, self.speed_grip, 1); time.sleep(0.1)
#                     for dz in (3.0, 1.5):
...
#                     mc.send_coords(goal_c, self.speed_approach, 0); wait_until_stop()
        ...
        try:
            while rclpy.ok() and self._ok:
                ...
                while rclpy.ok() and self._ok and (not self.is_order_done()):
                    ...
                    a_cmd = None
                    if angle_deg is not None:
    # (품질 체크를 생략하고 무조건 angle 계산)
                        if self._angle_prev is not None:
                            angle_deg = _unwrap_to_prev(angle_deg, self._angle_prev)
                            if abs(angle_deg - self._angle_prev) > self.angle_jump_limit:
                                log.info("큰 점프 → skip")
                                return None
                        ...
                        log.info(
        f'angle_raw={angle_deg:.1f}°, filt={angle_filt:.1f}°, cmd={a_cmd:.1f}° '
        + (f'(area={a_area:.0f}, ar={a_aspect:.2f})'
           if (a_area and a_aspect) else '(PCA)')
    )
